...
public class HumidityServiceThread extends Thread {
	...
	public void run() {
		...
		for(Row row :results)
			...
			try { 
				while(true){
				for (Integer i : al) { 
					DotNetClient.getHumidityFeq(IdProvider.getHumidityId(),i);
				    }
					//Thread.sleep(3000);
				Thread.sleep(DotNetClient.map.get("Humidity")*60000);
